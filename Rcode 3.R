...
plot_clusters_on_transition_matrix <- function(mc, clusters, title) {
  df <- as.data.frame(as.table(mc@transitionMatrix))
  colnames(df) <- c("From", "To", "Probability")
  df$ClusterFrom <- as.factor(clusters[df$From])
  df$ClusterTo <- as.factor(clusters[df$To])
...
run_simulation <- function() {
  ...
  # Plot clusters
  p5 <- plot_clusters(clusters, "Cluster Assignment of States")
  
  # Overlay clusters on transition matrix
  p6 <- plot_clusters_on_transition_matrix(original_mc, clusters, "Cluster Assignment on Original Transition Matrix")
  p7 <- plot_clusters_on_transition_matrix(reduced_mc, clusters, "Cluster Assignment on Reduced Transition Matrix")
  ...
  ggsave("cluster_assignment.png", plot = p5, width = 8, height = 6)
  ggsave("clusters_on_original_transition_matrix.png", plot = p6, width = 8, height = 6)
  ggsave("clusters_on_reduced_transition_matrix.png", plot = p7, width = 8, height = 6)
  ...
  grid.arrange(p5, p7, ncol = 2) # Cluster assignment and reduced transition matrix
}
