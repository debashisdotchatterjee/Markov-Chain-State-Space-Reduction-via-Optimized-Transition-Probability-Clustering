...
plot_state_probabilities_comparison <- function(original_probs, reduced_probs, clusters) {
  ...
  p <- ggplot() +
    geom_bar(data = original_df, aes(x = State, y = Probability, fill = Type), stat = "identity", color = "black", linetype = "dashed", width = 0.6) +
    geom_bar(data = reduced_df, aes(x = State, y = Probability, fill = Cluster), stat = "identity", alpha = 0.5, width = 0.6) +
    scale_fill_manual(values = c("Original" = "grey", rainbow(length(unique(clusters)))), guide = "none") +
    theme_minimal() +
    labs(title = "Comparison of Original vs Reduced State Probabilities", x = "State", y = "Probability") +
    theme(legend.position = "bottom", legend.title = element_blank())
  return(p)
}
