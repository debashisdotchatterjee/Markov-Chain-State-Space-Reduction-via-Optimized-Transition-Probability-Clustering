...
run_simulation <- function() {
  ...
  # Create transition matrix and Markov chain
  transition_matrix <- create_transition_matrix(birthwt$state)
  mc <- create_markov_chain(transition_matrix)
  
  # Reduce state space
  k <- 6
  reduced_result <- reduce_state_space(mc, k)
  reduced_mc <- reduced_result$mc
  clusters <- reduced_result$clusters
  
  # Compute steady-state distribution for reduced Markov chain and measure computational time
  start_time <- Sys.time()
  ...
  p1 <- plot_transition_matrix(reduced_mc, "Transition Matrix")
  ...
  p2 <- ggplot(df, aes(x = State, y = Probability, fill = Cluster)) +
    ...
    labs(title = "Steady-State Distribution", x = "State", y = "Probability", fill = "Cluster")
  
  # Overlay clusters on transition matrix
  p3 <- plot_clusters_on_transition_matrix(mc, clusters, "Cluster Assignment on Transition Matrix")
  
  # Plot clusters alone
  p4 <- plot_clusters(clusters, "Cluster Assignment of States")
  
  # Save plots
  ggsave("transition_matrix.png", plot = p1, width = 8, height = 6)
  ggsave("steady_state_distribution.png", plot = p2, width = 8, height = 6)
  ...
  # Store the computation time in a data frame for use in the LaTeX document
  performance_metrics <- data.frame(
    Metric = "Computation Time",
    Value = as.numeric(reduced_computation_time)
  )
